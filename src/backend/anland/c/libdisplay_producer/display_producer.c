...
/*
 * Receive the dmabuf set the consumer pushes onto the data channel. Replaces
 * any previously-held set (closing the old fds), so the consumer can re-allocate
 * its buffers (e.g. after a resize / occlusion) and hand us a fresh set without
 * a full reconnect. Polls data_fd with a short timeout. Returns 0 / -1.
 */
static int receive_dmabufs_inner(display_ctx *ctx)
{
...
/*
 * Initial dmabuf receive (handshake path). Does not replace an existing set.
 */
static int receive_dmabufs(display_ctx *ctx)
{
    if (ctx->buf_count > 0)
        return 0;
    return receive_dmabufs_inner(ctx);
}
...
int poll_input_event(display_ctx *ctx, struct InputEvent *event, int timeout_ms)
{
    ...
    /* The consumer re-allocated its dmabuf set (resize / occlusion). Replace our
     * set and tell the caller so it can re-import. Returns -2 = "buffers changed". */
